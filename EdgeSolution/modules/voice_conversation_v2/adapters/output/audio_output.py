...
class AudioOutputAdapter:
    ...
    async def speech_from_llm(self, text: str) -> None:
        """LLM streaming response audio output - uses real-time streaming"""
        try:
            self.logger.info("LLM response TTS: %s...", text[:30])
            if hasattr(self.tts_client, 'core_synthesizer'):
                self.logger.info("Real-time streaming synthesis start")
                first_chunk = True
                total_bytes = 0
                chunks_received = 0
                self._speaking.set()  # Mark TTS playback as active
                
                # Start streaming playback session
                if not self.audio_device.start_streaming_playback():
                    self.logger.error("Failed to start streaming playback session")
                    await self._play_apology(self.config_loader.get("error_messages.apology_not_heard"))
                    self._speaking.clear()
                    return
                
                try:
                    for audio_chunk in self.tts_client.core_synthesizer.synthesize_streaming_realtime(text):
                        if first_chunk:
                            self.logger.info("Real-time audio playback start")
                            first_chunk = False
                        
                        # Play audio chunk immediately as received
                        success = self.audio_device.play_bytes(audio_chunk)
                        if not success:
                            self.logger.error("Real-time audio chunk playback failed")
                            await self._play_apology(self.config_loader.get("error_messages.apology_not_heard"))
                            break
                    ...
                    if not first_chunk:  # If any chunk was played
                        self.logger.info(f"Real-time streaming completed: {chunks_received} chunks, {total_bytes} bytes")
                    else:
                        self.logger.error("No audio data received from streaming synthesis")
                        await self._play_apology(self.config_loader.get("error_messages.apology_not_heard"))
                        
                except TimeoutError as timeout_error:
                    self.logger.error(f"TTS synthesis timeout: {timeout_error}")
                    try:
                        if hasattr(self.audio_device, 'stop'):
                            self.audio_device.stop()
                    except Exception:
                        self.logger.debug("audio_device.stop failed", exc_info=True)
                    await self._play_apology(self.config_loader.get("error_messages.synthesis_timeout"))

                except Exception as streaming_error:
                    self.logger.error(f"Real-time streaming error: {streaming_error}")
                    await self._play_apology(self.config_loader.get("error_messages.apology_not_heard"))
                    
                finally:
                    # Stop streaming playback session and wait for completion
                    self.audio_device.stop_streaming_playback()
                    # Add small delay to ensure audio finishes playing
                    await asyncio.sleep(0.5)
                    self._speaking.clear()  # Mark TTS playback as finished
            else:
                # Fallback to single synthesis if streaming unavailable
                await self.speech_announcement(text)
        except Exception as e:
            self.logger.error(f"LLM speech synthesis error: {e}")
